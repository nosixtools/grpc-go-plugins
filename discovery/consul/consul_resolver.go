...
func NewConsulBuilder(address string, tags string, opts *consulapi.QueryOptions) resolver.Builder {
	config := consulapi.DefaultConfig()
	config.Address = address
	client, err := consulapi.NewClient(config)
	if err != nil {
		log.Fatal("consul: create consul client error", err.Error())
		return nil
	}
...
func (cb consulBuilder) resolve() ([]resolver.Address, string, error) {
	...
	serviceEntries, _, err := cb.client.Health().Service(cb.serviceName, cb.tags, true, cb.opts)
	if err != nil {
		return nil, "", err
	}
...
func NewResolverOpts(address, serviceName, tags string, opts *consulapi.QueryOptions) *resolverOpts {
	return &resolverOpts{address: address, serviceName: serviceName, tags: tags, opts: opts}
}

func GenerateAndRegisterConsulResolver(resolverOpts *resolverOpts) (schema string, err error) {
	builder := NewConsulBuilder(resolverOpts.address, resolverOpts.tags, resolverOpts.opts)

	target := resolver.Target{Scheme: builder.Scheme(), Endpoint: resolverOpts.serviceName}
